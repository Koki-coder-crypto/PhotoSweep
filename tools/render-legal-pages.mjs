...
const pages = {
  ...
  terms: ['利用規約・購入条件', [
    ...
    ['料金と更新', 'Proには自動更新プランと、1回の支払いで利用できる買い切りプランがあります。価格・通貨・対象者の無料体験は、購入時のApp Store画面に従います。無料体験は対象の方だけに適用されます。継続しない場合は終了の24時間以上前にApp Storeで解約してください。購入と請求はAppleが処理します。'],
  ...
  support: ['ヘルプ・お問い合わせ', [
    ...
    ['購入・無料枠について', '新しい写真の仕分けは無料で1日50枚。候補の確認・削除・取り消し・保存はいつでも無料です。Proの管理や復元は設定から開けます。購入の保留や確認失敗では、再購入の前にApp Storeの状況を確認してください。'],
...
for (const [slug, [title, sections]] of Object.entries(pages)) {
  const html = `<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PhotoSweep · ${title}</title><style>body{margin:0;background:#070B19;color:#F5F7FF;font:16px/1.9 system-ui,sans-serif}main{max-width:720px;margin:auto;padding:40px 24px}nav{display:flex;gap:20px;flex-wrap:wrap}a{color:#65CFFF}h1{font-size:30px;line-height:1.4}h2{font-size:20px;margin-top:32px}aside{padding:16px;background:#332D20;border-radius:16px}section{background:#141A2B;padding:8px 24px 16px;border-radius:24px;margin-top:16px}footer{margin-top:40px;font-size:13px;color:#A4AEC5}</style><main><nav><b>PhotoSweep</b><a href="support.html">ヘルプ</a><a href="privacy.html">プライバシー</a><a href="terms.html">規約</a></nav><h1>${title}</h1>${draft ? '<aside>公開前の原稿です。運営者と問い合わせサービスの情報を確定してから公開します。</aside>' : ''}${sections.map(([heading, text]) => `<section><h2>${escape(heading)}</h2><p>${escape(text)}</p></section>`).join('')}<footer>原稿更新：2026年9月17日</footer></main></html>`;
