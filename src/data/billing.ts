    ...
    async (purchase) => {
      if (
        ![
          ...
          config.products.annual,
          config.products.legacy,
          ...
          entitlementFromStore(
            {
              ...
              legacy: sku === config.products.legacy,
            },
            Date.now(),
          ),
        );
  ...
  return {
    async loadProducts() {
      ...
      const products = await IAP.fetchProducts({
        skus: [config.products.monthly, config.products.annual],
        type: "subs",
      });
      const result: StoreProduct[] = [];
      for (const raw of products || []) {
        if (raw.platform !== "ios" || raw.type !== "subs") continue;
    ...
    async purchase(sku) {
      ...
      if (![config.products.monthly, config.products.annual].includes(sku))
