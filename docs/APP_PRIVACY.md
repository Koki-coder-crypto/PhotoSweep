...
実装には広告SDK、外部分析SDK、独自サーバー、写真アップロード、ユーザーアカウントがありません。写真識別子、判断、進行、無料枠、整理履歴、設定は端末内SQLiteに保存します。表示画像のキャッシュは端末内で扱います。類似画像の指紋と原画像のSHA256は端末内で計算し、メモリに保持します。位置情報を解析しません。解析のためのiCloudダウンロードは行いません。
